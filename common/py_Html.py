...
def passfail(tend):
    ...
    return htl
def ceshixiangqing(id,name,token,coneent,url,meth,yuqi,json,relust):
    xiangqing='''
        <tr>
            <td>%s</td>
            <td>%s</td>
    ...
    '''%(id,name,token,coneent,url,meth,yuqi,json,passfail(relust))
    return xiangqing
weibu='''
	</table>
    </body>
    </html>'''
def relust(titles,starttime,endtime,passge,fail,id,name,token,coneent,url,meth,yuqi,json,relust,exceptions,weizhi):
    if type(name) ==list:
        relus=' '
        for i in range(len(name)):
            relus+=(ceshixiangqing(id[i],name[i],token[i],coneent[i],url[i],meth[i],yuqi[i],json[i],relust[i]))
        text=title(titles)+connent+shouye(starttime,endtime,passge,fail,exceptions,weizhi)+shanghai+relus+weibu
    else:
        text=title(titles)+connent+shouye(starttime,endtime,passge,fail,exceptions,weizhi)+shanghai+ceshixiangqing(id,name,token,coneent,url,meth,yuqi,json,relust)+weibu
    return text

def createHtml(filepath,titles,starttime,endtime,passge,fail,id,name,token,coneent,url,meth,yuqi,json,relusts,exceptions,weizhi):
    texts=relust(titles,starttime,endtime,passge,fail,id,name,token,coneent,url,meth,yuqi,json,relusts,exceptions,weizhi)
